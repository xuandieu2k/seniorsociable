 ...
 * @Date: 27 / 12 / 2023
 */
class PostModerationAdapter(context: Context) : AppAdapter<Post>(context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AppViewHolder {
        val binding =
            ItemPostModerationBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ViewHolder(binding)
    }

    inner class ViewHolder(private val binding: ItemPostModerationBinding) :
        ...
        private fun setStatusPost(item: Post) {
            when(item.status)
            {
                PostConstants.IS_PASSED ->{
                    binding.imvStatus.setImageResource(vn.xdeuhug.seniorsociable.R.drawable.ic_post_is_passed)
                    binding.imvStatus.setColorFilter(ContextCompat.getColor(getContext(), vn.xdeuhug.seniorsociable.R.color.green_008))
                    binding.tvStatus.text = getString(vn.xdeuhug.seniorsociable.R.string.is_passed)
                    binding.tvStatus.textColor = getColor(vn.xdeuhug.seniorsociable.R.color.green_008)
                }
                PostConstants.IS_NOT_PASSED ->{
                    binding.imvStatus.setImageResource(vn.xdeuhug.seniorsociable.R.drawable.ic_post_is_not_passed)
                    binding.imvStatus.setColorFilter(ContextCompat.getColor(getContext(), vn.xdeuhug.seniorsociable.R.color.red_600))
                    binding.tvStatus.text = getString(vn.xdeuhug.seniorsociable.R.string.is_not_passed)
                    binding.tvStatus.textColor = getColor(vn.xdeuhug.seniorsociable.R.color.red_600)
                }
                PostConstants.IS_PENDING_PASSED ->{
                    binding.imvStatus.setImageResource(vn.xdeuhug.seniorsociable.R.drawable.ic_post_is_pending_passed)
                    binding.imvStatus.setColorFilter(ContextCompat.getColor(getContext(), vn.xdeuhug.seniorsociable.R.color.orange_700))
                    binding.tvStatus.text = getString(vn.xdeuhug.seniorsociable.R.string.is_pending_passed)
                    binding.tvStatus.textColor = getColor(vn.xdeuhug.seniorsociable.R.color.orange_700)
                }
