 ...
 * @Date: 10 / 11 / 2023
 */
class InputOTPLoginActivity : AppActivity() {
    ...
    private var userAuthCurrent = User()
    ...
    override fun initView() {
        ...
        userAuthCurrent =
            Gson().fromJson(intent.getStringExtra(AppConstants.PERSONAL_USER), User::class.java)
        phoneNumber = userAuthCurrent.phone
    ...
    private fun enableOTPView(isEnable: Boolean) {
        if (!isEnable) {
            ...
            showKeyboard(this@InputOTPLoginActivity)
        }
    }
    ...
    private fun goToHome() {
        try {
            UserCache.saveUser(userAuthCurrent)
            hideDialog()
